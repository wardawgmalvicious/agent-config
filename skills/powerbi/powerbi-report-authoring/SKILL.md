...
## Local vendoring note (not part of the upstream skill)
...
- `powerbi-report-planning` is **not installed** — use `pbir-report-workflow`
  for end-to-end requirements/build sequencing instead.
- `powerbi-report-management` is **not installed** — use `fabric-rest-api` /
  `fabric-tmdl-api` (and `pbir-cli` publish verbs) for Fabric definition
  transport.
- The Power BI Desktop Bridge loop (`powerbi-desktop` open/reload/screenshot)
  is **skipped in this environment**: the workflow here is service-first with
  Fabric Git sync, and the primary models are Direct Lake, where opening a
  PBIP forces remote modeling and risks overwriting the live workspace model.
  For rendered visual verification use the service-side `exportToFile` PNG
  loop instead — see `pbir-report-workflow` Step 12.
- `powerbi-report-author` CLI (npm `@microsoft/powerbi-report-authoring-cli`,
  Node >= 20) IS in scope: its offline `validate` and `catalog` / `formatting`
  introspection commands work purely on files and complement `pbir-cli`.
