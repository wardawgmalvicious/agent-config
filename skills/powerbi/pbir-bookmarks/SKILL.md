...
when_to_use: Invoke when the user adds a bookmark, builds show/hide toggle buttons, or debugs 'bookmark applies but nothing changes'. Bookmarks embed page and visual state, so use this skill whenever the edit target is a bookmark file — not pbir-filters, even though the bookmark carries filters.byExpr, and not pbir-visual-json, even though it carries visualContainers. Reach for those two only when editing the live page or visual files themselves.
paths:
