...
when_to_use: Invoke when the user edits a theme file, tweaks CY24SU10, or fixes a theme-not-applied issue. This skill owns report-wide defaults and filter-pane styling. Theme JSON uses bare numbers and bare hex, unlike visual.json — so for a per-visual override with D/L/M literal suffixes use pbir-visual-json, and for per-point colour driven by a measure use pbir-conditional-formatting.
paths:
