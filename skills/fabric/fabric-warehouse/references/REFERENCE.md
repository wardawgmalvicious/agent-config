...
- [Git integration for Fabric warehouse development](https://learn.microsoft.com/fabric/data-warehouse/git-integration) — item definition **2.0** (Aug 2026), DacFx incremental extraction, and the authoritative *Limitations in Git integration* list. (The older `/source-control` URL now redirects to `development-deployment`.)
- [Troubleshoot Git integration for warehouse development](https://learn.microsoft.com/fabric/data-warehouse/troubleshoot-git-integration) — the DacFx build-time failures that pass `sqlcmd` and fail the sync: three-part self-references, unqualified cross-warehouse columns, schema-name capitalisation, `::` ambiguous columns, stale `.sqlproj` SDK pin, silently-dropped column collation.
- [Upgrade the system file version of a warehouse](https://learn.microsoft.com/fabric/data-warehouse/upgrade-warehouse) + [system file release notes](https://learn.microsoft.com/fabric/data-warehouse/warehouse-system-file-version-history) — the *Apply system updates* procedure (commit first; upgrades every warehouse in the workspace at once) and what 2.0 changed.
- [Deploy a warehouse using pipelines](https://learn.microsoft.com/fabric/data-warehouse/deploy-pipelines) — the fixed DacFx settings (`BlockOnPossibleDataLoss`, `DropObjectsNotInSource`, `IncludeTransactionalScripts`, `GenerateSmartDefaults`, `ExcludeObjectTypes`) and the success-with-skipped-drop limitation.
