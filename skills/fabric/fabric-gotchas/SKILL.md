...
| Table empty after Git sync of a DDL change (warehouse definition **1.0**) | Sync rebuilt the table (DacFx table-rebuild path) — even a nullable `ADD COLUMN` (observed 2026-09-03; mechanism undocumented). A 2.0 warehouse refuses instead — next row | Capture needed rows/state before syncing DDL to a populated table; re-register after. See fabric-warehouse — *Schema Evolution* |
| Git sync or pipeline deploy fails `DataLoss: The column [s].[t].[c] is being dropped` + `Msg 50000 ... Rows were detected. The schema update is terminating` (definition **2.0**) | DacFx runs with `BlockOnPossibleDataLoss = true`, not configurable. The generated guard is `IF EXISTS (SELECT TOP 1 1 FROM [s].[t])`, so any row in the table trips it — an all-NULL dropped column doesn't help | Capture what you need, then `ALTER TABLE s.t DROP COLUMN c` on the live warehouse **before** syncing, so the plan carries no drop. See fabric-warehouse — *Schema Evolution* (observed Sept 2026; documented for pipelines) |
...
| `520` / `Internal service error` / `"@permanent": false` from a Kusto admin command, with client advice to retry after backoff | A state check reported through the generic internal-error envelope — the envelope's own metadata and retry advice are wrong. Commonly `.rename tables` against tables with OneLake availability on | Read the **nested** `@message` before believing `@permanent`: *"Tables cannot be renamed as at least one table has mirroring policy enabled."* The command fails atomically, so treat it as a clean no-op and fix the state. Not the cold-table `520` below, which really is transient. See [[fabric-eventhouse]] — *Schema Evolution* |
| New column lands empty after a `.create-merge`, no error, success response with matching record count | Streaming ingestion is writing the cached schema; the refresh is automatic but takes minutes | `.clear table <T> cache streamingingestion schema`, or `.clear database cache …` for every table in one call. Also collapses the cold-table window in which a new table answers `520` to every ingest. See [[fabric-eventhouse]] — *Ingestion* |
| `InvalidShortcutPayloadBatchErrors`, "Target path doesn't exist" on a Lakehouse git sync | A OneLake shortcut names a target table that does not exist — this fails the **entire** Lakehouse update, not just that shortcut | Add a shortcut only after its target exists. Don't let a generator emit shortcuts from a declaration list: a contract can legitimately declare a table before it is created, and the shortcut file is where that gap becomes an outage |
| `SQL71501 ... unresolved reference ... [dbo].[SYSNAME]` failing a whole warehouse import on branch-out | Uppercase built-in type name under a case-sensitive model collation — DacFx resolves it as a missing UDT. The engine stores built-in type names lowercase, so uppercase in a repo file is always a divergence from the live object | Write built-in type names lowercase (`sysname`). It executes fine over sqlcmd, so it only surfaces at the DacFx build — which runs on branch-out and new-workspace-from-repo, not on ordinary syncs. See `coding-tsql` — *Casing* |
| `Incorrect syntax near '.'` importing a warehouse, from a generated `CREATE SCHEMA .sharedqueries;` | A workspace still on warehouse definition **1.0** read a `.sharedqueries` folder as a schema. Under **2.0** that folder at the project root is the documented home of every shared query — not a stray, and deleting it on a 2.0 warehouse deletes real queries | Upgrade the workspace (**Apply system updates** in Source control; every warehouse in it at once) so it understands the folder. Only where the workspace must stay on 1.0: remove the folder from that branch, recovering the scripts from git. Keep hand-authored `.sql` out of `*.Warehouse` folders either way — see [[fabric-warehouse]] — *Warehouse Authoring Rules* |
| `MSB4019`, missing `NuGet.Build.Tasks.Pack.targets`, building a warehouse `.sqlproj`; or a Git commit/update failing on `IDENTITY` / `CLUSTER BY` syntax | Stale `Microsoft.Build.Sql` SDK pin (`0.1.x`) in a repo committed before the warehouse moved to definition 2.0 (which pins `2.3.0-preview.1`). Not a migration-window blip — it recurs on every path that builds from the repo with no warehouse to sync from: new workspace ← existing repo, branch-out, restoring a deleted warehouse, or syncing from git right after the upgrade | Where the warehouse exists and is healthy, commit workspace → git first (*option A*) — **but it must already be on 2.0**, or the commit writes 1.0 back and the next sync fails identically. Otherwise edit the `.sqlproj` SDK version to the current NuGet release and commit (*option B*). An export or diff never rewrites the file |
| Deployment pipeline reports success, but a table dropped in source is still present in the target | `DropObjectsNotInSource = false` is fixed for warehouse deploys, and a skipped drop-table doesn't fail the run — the target silently drifts from source control | Drop it by hand on the target, or accept the drift. Check *Compare* after any deploy that removed an object; the green tick doesn't cover drops |
