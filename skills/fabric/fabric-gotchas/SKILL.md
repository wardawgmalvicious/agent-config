...
| `Incorrect syntax near '.'` importing a warehouse, from a generated `CREATE SCHEMA .sharedqueries;` | A workspace on the older warehouse definition format misread a leftover `.sharedqueries` folder as a schema | Delete the folder and recover the scripts from git rather than re-authoring them. Keep hand-authored `.sql` out of `*.Warehouse` folders entirely — see [[fabric-warehouse]] — *Warehouse Authoring Rules* |
| `MSB4019`, missing `NuGet.Build.Tasks.Pack.targets`, building a warehouse `.sqlproj` | Stale `Microsoft.Build.Sql 0.1.19-preview` pin, written back by a workspace still on the older warehouse definition format | Bump the SDK version in the repo's `.sqlproj` — Microsoft's documented *fix option B* — or, where the warehouse exists and is healthy, commit workspace → git first (*option A*). Migration-window issue: expect it to disappear as the current definition format finishes rolling out (observed Sept 2026) |
