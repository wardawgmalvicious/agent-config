---
name: 'Spark SQL Conventions'
description: 'Spark SQL conventions for Fabric Lakehouse, Databricks SQL and notebook %%sql blocks'
applyTo: '**/*.Notebook/notebook-content.sql,**/*.SparkJobDefinition/**/*.sql,**/notebooks/**/*.sql,**/lakehouse/**/*.sql,**/spark/**/*.sql'
---
...
## Precedence over `coding-tsql.instructions.md` — read the kernel first

`coding-tsql.instructions.md` globs a bare `**/*.sql`, so it also
matches every file this rule matches. The two contradict — brackets vs
backticks, `ISNULL` vs `COALESCE`, three-part `dbo.` naming vs
catalog/schema paths — so one
...
| `sqldatawarehouse` | `sql` | T-SQL on a Warehouse | `coding-tsql.instructions.md` |
...
rule wins and T-SQL conventions do not apply — ignore
`coding-tsql.instructions.md` rather than blending the two. On a
`sqldatawarehouse` notebook, defer to `coding-tsql.instructions.md`;
that engine really does want `[brackets]` and `dbo.` qualification.

An `applyTo` glob cannot make this call, because the kernel lives inside
the file. That is also why the overlap is resolved in prose rather than
by narrowing `coding-tsql.instructions.md`'s glob — and narrowing it
would break the T-SQL rule in ordinary non-Fabric SQL repos, which is
what that wide
glob is for.
