---
name: 'T-SQL Conventions'
description: 'T-SQL style and correctness conventions for SQL Server, Azure SQL, Fabric Warehouse and Synapse SQL pools'
applyTo: '**/*.sql'
---
...
- **Spark SQL** — a `<Name>.SparkJobDefinition/**/*.sql` is Spark SQL,
  ...
  **either**: the file name is the same for both dialects, and the
  ...
  to `coding-sparksql.instructions.md`, which also matches that path
  and wins, and drop the
...
- **Fabric SQL Database** — a `<Name>.SQLDatabase/**/*.sql` *is* Azure
  ...
  not apply there — see the `fabric-database` skill.
...
- **Built-in *type* names**: lowercase — `sysname`, `int`, `varchar`.
  ...
  the whole DacFx build with `SQL71501`. It executes fine over sqlcmd,
  so it surfaces only at build time — on branch-out and
  new-workspace-from-repo.
