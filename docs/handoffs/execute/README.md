...
| Item | State |
| --- | --- |
| [worktree-isolation-scope.md](worktree-isolation-scope.md) | **Open, written 2026-09-22, reduced 2026-09-24 to its probe**, which the user runs in a fresh `claude --worktree probe-isolation`. What the brief argued has landed: root's § "Branching and concurrent sessions" scopes "only a commit isolates" to one tree, says a worktree buys **the index, not the queue**, and closes its `.claude/skills/` flag from the docs, with `.claude/rules/` by inference, marked unprobed; `.claude/settings.json` sets `worktree.baseRef` to `"head"`, this repo only, since `fresh` silently drops unpushed commits. The user kept `claude/CLAUDE.md`'s escalation paragraph whole. The evidence, both decisions and what stays ruled out (agent teams) are the 2026-09-24 entry closing that heading in `docs/evidence/root-claude-md.md`. The probe runs before a push, with a commit `origin/main` lacks, and asserts the base, the four enforcement checks one at a time, two paths the docs leave outside them, which copy of a skill and a rule loads, and whether root `CLAUDE.md` loads twice; its result goes to the ledger, root's line loses "unprobed" or is corrected, and the brief and this row go in that commit. |
| [nested-instruction-files.md](nested-instruction-files.md) | **Open, written 2026-09-24.** When a repo should get a nested `CLAUDE.md` or `AGENTS.md`, this repo included but nothing under `claude/`, and possibly a skill that decides it in any repo, deeper than `/init`. Carries what that day established: a nested file loads on the first Read beneath it, never at launch; Claude reads `AGENTS.md` only when no `CLAUDE.md` sits on the path, a default that only user or managed settings can change; and in a client Fabric repo, 5 of 9 times a session worked in a folder it opened that folder's README late or never. Seven load-order inferences need cold probes before any strategy is written down. Three decisions are the user's: a skill at all, where the strategy lives, and the machine's `instructionFiles` setting. |
