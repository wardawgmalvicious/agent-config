...
Four briefs are open. Fourteen waves are spent (struck through below).
...
deleted **individually** as each is spent, and heavily cross-linked — 35
relative markdown links across the current 5 files, counting this queue
(recounted 2026-09-03 when wave 18 retired `land.md`, by the method kept
with the number: `](` occurrences that are not `http`. The lineage runs 57
...
it again; earlier figures of 27 and 41 do not reproduce under any method
tried, so it starts at 57).
Numbering the
...
late only because it was expensive. Of what remains, **7, 16 and 17 block
...
there, so its carve-out debt is **deferred, not discharged** — it stays
owed to a shipped skill rather than a brief, which is what makes it
concrete: the audit exists today to be wrong about a planning model, and
will be for as long as the defer holds. That is the cost of the defer and
it is accepted, because a model it would misjudge does not exist here
either. Added 2026-09-02 from a semantic-model coverage question.
