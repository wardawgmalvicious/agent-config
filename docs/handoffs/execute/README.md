...
| [prune-branches-skill.md](prune-branches-skill.md) | **Open, written 2026-09-14** from a live six-branch cleanup in a client repo. A `workflow`-group skill that audits every branch a repo carries and sorts them into safe, stale, and carrying unmerged work. Exists because the obvious tools are wrong in a squash-merge repo: `git branch --merged` and `git cherry` both reported squash-merged branches as unmerged, so merge evidence has to come from the PR list, and the run's one real find was a branch whose unique work every commit-level heuristic rated the same as two branches holding nothing. Fully drilled from that run; nothing further to verify before drafting. Scoped **audit-only** for a structural reason recorded in the brief — the auto-mode classifier denies `git push origin --delete`, so a delete-capable skill would fail its last step every time. One stated limit, not a gap: only the squash-merge GitHub path is evidence-backed, and the outline handles the rest by detecting merge style rather than claiming coverage. Boundary with `land` is explicit and non-overlapping. |
