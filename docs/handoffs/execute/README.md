...
Nine items remain on the queue: five open, four deferred.

| Item | State |
| --- | --- |
| [az-tenant-model-fallout.md](az-tenant-model-fallout.md) | **Open, written 2026-09-14**; item 2 discharged 2026-09-14, item 1 still to decide. Out of the machine-config session that replaced `az account clear` with per-tenant `AZURE_CONFIG_DIR` directories. Two independent items, either takeable alone. The open one is a real decision: `link-claude.ps1` deploys `CLAUDE.md` and `settings.json` as copies behind the **same** `-Force`, so the one flag needed to push a CLAUDE.md edit also overwrites `settings.json` — and the live file carries four `/config`-owned keys (`theme`, `agentPushNotifEnabled`, `tui`, `model`) the repo copy does not, which a documented-default run would have discarded silently. The script has no `-WhatIf`, so only diffing first finds it. Brief recommends the `vscode-profiles.ps1` re-merge precedent and records why folding the keys into the repo copy was rejected. Item 2 was prose: this row and `claude/mcp/README.md:185`, both corrected 2026-09-14. Its original citation — a hosted-MCP probe row "immediately below" this one — named text that exists nowhere in the repo and is retracted in the brief; the forward-looking advice it meant is in the 2026-09-12 audit record, deliberately left as history. What replaced the premise: a tool shell loads no profile, so it never receives the tenant pin and `az` there reads the shared `~/.azure` — an equivalent failure by a different route, not a failure mode that is gone. |
| [branch-default-hardening.md](branch-default-hardening.md) | **Prose edit landed 2026-09-14; deploy pending, one decision open.** Out of a machine-config session asked where its branching habit came from. The answer was Claude Code's built-in Bash-tool prose, which is advisory, unconditional and gated by nothing, while `claude/CLAUDE.md` § "Branch naming" specified how to name a branch and never when to make one. The landed clause is deliberately **narrower than the harness rule** — it relaxes a blanket "branch first" into more-than-one-commit-or-broken-intermediate-state, and says so in the payload, so nobody reads it as a tightening. It sits under the existing repo-precedence sentence, which is load-bearing: this repo's own § "Branching and concurrent sessions" is a dated opt-out that must stay correct, and now points back at the payload from the overriding side. **What remains**: deploying needs `link-claude.ps1 -Force`, so it inherits [az-tenant-model-fallout.md](az-tenant-model-fallout.md) item 1 — diff `settings.json` first, or settle that item; and the open decision of whether to follow with a `PreToolUse` hook refusing a commit on the default branch, on the `identity-guard.sh` precedent. Lean is prose-first, reaching for the hook only if a lapse recurs. The hard part there is the override path, since this repo's own convention is exactly the legitimate single-commit case. Sibling brief in fabric-tools covers the repo that has no policy either way. |
...
| [skill-overlap-script.md](skill-overlap-script.md) | **Open, written 2026-09-10.** A script for the one thing no existing tool measures: skills as **pairs** — a description routing to a skill that is not installed, two descriptions half-matching one request, two globs that always fire together — joined with `skill-telemetry.py` usage. Its cheapest signal's first run found both vendored `powerbi-report-*` descriptions routing to `powerbi-report-planning`, which is not installed — an override kept on purpose, settled 2026-09-11 against `1fa3061`, so the check reports it as accepted rather than failing; 11 of the 13 raw hits were noise, which is why the brief derives an allowlist. **Fix `skill-telemetry.py` first** — its inventory walks `skills/` only, so the seven project-scope skills can never be flagged. Feeds `author-skill` §2 and `learn` Step 4 rather than adding a trigger. One design fork (new script vs a `payload-coverage.py` subcommand) is left to execution, with a lean. |
