...
| Item | State |
| --- | --- |
| [az-tenant-model-fallout.md](az-tenant-model-fallout.md) | **Open, written 2026-09-14**, out of the machine-config session that replaced `az account clear` with per-tenant `AZURE_CONFIG_DIR` directories. Two independent items, either takeable alone. The first is a real decision: `link-claude.ps1` deploys `CLAUDE.md` and `settings.json` as copies behind the **same** `-Force`, so the one flag needed to push a CLAUDE.md edit also overwrites `settings.json` — and the live file carries four `/config`-owned keys (`theme`, `agentPushNotifEnabled`, `tui`, `model`) the repo copy does not, which a documented-default run would have discarded silently. The script has no `-WhatIf`, so only diffing first finds it. Brief recommends the `vscode-profiles.ps1` re-merge precedent and records why folding the keys into the repo copy was rejected. The second is prose: `docs/handoffs/execute/README.md` (the hosted-MCP probe row, immediately below) and `claude/mcp/README.md:180` still say an interactive shell's profile clears the Azure account — the first as forward-looking advice, which would send a probe session chasing a failure mode that no longer exists. Two dated audit records also cite it and are deliberately left as history. |
