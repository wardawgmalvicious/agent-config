...
| [skill-overlap-script.md](skill-overlap-script.md) | **Open, written 2026-09-10.** A script for the one thing no existing tool measures: skills as **pairs** — a description routing to a skill that is not installed, two descriptions half-matching one request, two globs that always fire together — joined with `skill-telemetry.py` usage. Its cheapest signal's first run found both vendored `powerbi-report-*` descriptions routing to `powerbi-report-planning`, which is not installed — an override kept on purpose, settled 2026-09-11 against `1fa3061`, so the check reports it as accepted rather than failing; 11 of the 13 raw hits were noise, which is why the brief derives an allowlist. **Re-measured 2026-09-15: 19 raw hits, the two real ones unchanged** — all five new names are noise, and four of them are counterfactual skill names in `skills/README.md`'s naming rationale, a class the brief treats as a one-off and which has no machine source to derive an allowlist from. Its prerequisite, the `skill-telemetry.py` inventory, **landed 2026-09-15**: it cost more than the one-function estimate the brief budgeted, because scope changes what a listing count means, and it immediately flagged `drift-audit` and `drift-update` as `SLASH-ONLY` — findings no earlier run could produce. Feeds `author-skill` §2 and `learn` Step 4 rather than adding a trigger. One design fork (new script vs a `payload-coverage.py` subcommand) is left to execution, with a lean. |
