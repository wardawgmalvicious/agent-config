...
| Item | State |
| --- | --- |
| [claude-md-double-load.md](claude-md-double-load.md) | **Open, written 2026-09-24** at the close of the root `CLAUDE.md` trim, which left it out as a separate change. A session here loads `~/.claude/CLAUDE.md` at launch, and again as the repo's `claude/CLAUDE.md` — byte-identical that day — the first time it Reads a file under `claude/`, since a subdirectory's `CLAUDE.md` loads on demand. Witnessed in two probe transcripts on 2.1.281, and in the `InstructionsLoaded` hook log, whose 15 `nested_traversal` loads since 2026-08-28 are **every one this file**. The fix is one `claudeMdExcludes` entry in `.claude/settings.json`, `**/claude/CLAUDE.md`, and **the test is the whole brief**: the docs match patterns against absolute paths and say nothing of Windows, while every path the log records is backslashed and its drive letter reads `c:` and `C:` by turns. Variants run through `--settings`, so the committed file changes only once one passes, and a pass needs the nested load gone, the rules still attaching, and root and user `CLAUDE.md` still loading at launch; if none passes, it goes back to the user with each fallback's cost. |
| [worktree-isolation-scope.md](worktree-isolation-scope.md) | **Open, written 2026-09-22**, from drilling the worktrees, cross-session-messaging and agent-teams docs; **repointed 2026-09-24** at the trimmed root, whose passages it quotes now sit in `docs/evidence/root-claude-md.md`, and whose own line already scopes the verdict to payload, so half the scope correction is done. The standing *"either unnecessary or ineffective, with no case in between"* verdict is **right and scoped to a different axis**: measured 2026-09-02 against `git worktree add` plus `link-claude.ps1`, it asked whether a worktree isolates *payload*, and the answer is still no because user scope outranks project scope. `--worktree`/`EnterWorktree` is a different mechanism carrying **enforcement** the hand-rolled worktree had none of — four checks over the session and every subagent it spawns, blocking edits into the main checkout, commands whose cwd resolves there, git redirected there by `-C`/`--git-dir`/`GIT_DIR`/a prior `cd`, and any command whose text cannot be verified to keep git inside, that last one **not disableable**. What it buys is stated precisely: **the index, not the document.** Each worktree has its own index, so the 2026-09-12 loss of a `--stamp` entry *"from the working tree and the index while it sat staged"* cannot recur, and root's "only a commit isolates" needs scoping to one tree. It does **not** buy the shared document: two worktrees are two branches and two divergent copies of this file, and `--ff-only` integration in a repo with no merge commit in 324+ commits turns a silent overwrite into a forbidden merge. Right tool for divergent state, wrong one for convergent; the convergent half was the queue's count line, deleted 2026-09-23 in `7d31355`. **Closes the section's open flag without a probe**: `.claude/skills` is tracked here (9 files) and a worktree holding its own copy loads only that copy, so the five project-scope skills *do* isolate, as does `.claude/rules/`, tracked since 2026-09-24, and the in-between case the 2026-09-02 verdict calls impossible is real — derived from docs plus one `git ls-files`, not from a run, and the read-through half wants 2.1.277, which this machine passed by 2026-09-24 (2.1.281). Preconditions measured clean: no `includeIf` in this repo's own `.git/config` (a local one refuses creation, and the error reads as a git-identity problem; the machine's is global and exempt), `[lfs]` carrying only `repositoryformatversion`, and neither `.claude` nor `.claude/skills` a symlink. One payload **decision** with a different clock: `worktree.baseRef` is unset everywhere so it defaults to `fresh`, and `origin/main` lags `HEAD` whenever pushes do (two commits on 2026-09-22, four through the 2026-09-24 trim), so a worktree cut then silently omits them — `head` is right here, `fresh` may be right in a client repo, and where to set it costs something either way. Agent teams **ruled out** on the docs' own *"two teammates editing the same file leads to overwrites"*, plus unasked team formation, one-session scope, and no split panes in VS Code's terminal. Cross-session messaging kept one non-payload thing, `--name`/`/rename`, since the `<cwd-basename>-<hash>` naming is only the fallback, and `claude/CLAUDE.md` took it on 2026-09-23 in `ed8434d`. |
