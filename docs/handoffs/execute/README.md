...
| [leak-remediation-correction.md](leak-remediation-correction.md) | **Open 2026-09-08** — part edit, part decision; do not execute part 3. Parts 1–2 correct `claude/CLAUDE.md` and the `identity-guard` comments, which name `refs/pull/N/head` as the *whole* reason a pushed leak survives a rewrite. It is half: even at zero PRs, GitHub serves unreachable objects by SHA until it GCs, which is why 2026-09-08 needed a second delete-and-recreate after a clean force-push. **This brief's central evidence cannot be re-measured** — the old SHAs are gone and a `404` today is the fix having worked, not a refutation; the brief opens with that warning, so read it before probing. Part 3 asks whether the commit-map SHA-translation trap is recorded at all and where, against a stated user objection to `claude/CLAUDE.md`'s length — put it back to the user. |
