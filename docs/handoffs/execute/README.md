...
Three briefs are open. Sixteen waves are spent (struck through below).
...
deleted **individually** as each is spent, and heavily cross-linked — 34
relative markdown links across the current 4 files, counting this queue
(recounted 2026-09-03 when wave 17 closed, by the method kept with the number:
...
were blocked by nothing, **only 7 is left** — 16 was gated solely on a
credential and the session that had one closed it, and 17 closed on
2026-09-03. Take 7 when a session fits it; nothing sequences it.
