...
| [handoff-convention-cross-repo.md](handoff-convention-cross-repo.md) | **Open, written 2026-09-16.** Generalize this repo's handoff discipline to the two other repos that have started growing handoff directories on their own — `machine-config` (private, one brief, created 2026-09-16 by an outbound `/learn` run, and **indexed to this brief's Q1 shape the same day** — the first worked instance) and a client estate repo (internal, three briefs, no index), the latter likely to be the second-highest-churn repo here. Measured 2026-09-16: the brief *shape* is already converging unprompted — all three estate briefs carry a `**Status:**` line that nobody standardized — so the gap is **the index**, which is the part this repo learned the hard way (order in exactly one file, no positions in filenames, a spent row invites re-execution). Proposes a minimal eight-point core plus a per-repo index and **explicitly not a shared template**, which could not span skill authoring, shell config and estate work without collapsing into generic headings. Two rules are new because briefs now cross repo boundaries: **scrubbing is decided by the destination's visibility, not the origin's**, and direction — local / outbound / inbound — has to be declared per directory, which none does today. **Q1 and Q2 were answered 2026-09-16**, the day the brief was written, and the brief's own argument against the rule form is **retracted** in its Decisions section — three of the eighteen rules are already "what belongs where" conventions. The rule form is ruled out on activation mechanics instead: `defaultMode` is `auto`, activation is keyed to the `Read` tool so `cat` fires nothing, and the trigger here is *writing* a brief into a directory that holds none — which no glob can see. Q1's answer is to **extend `/learn`** rather than add any of the three proposed forms: invariants into `skills/meta/learn/references/`, the hand-written `~/.copilot/instructions/cross-repo-handoffs.instructions.md` ported into `copilot/instructions/` so it is versioned and reaches both harnesses, and a **stub** README per handoff directory carrying direction, visibility and the index only. A `/handoff` skill was declined on measured overlap — `author-skill + learn` is already the top pair at 32.88. Q2: the inbox **widens**, routed by a directory per destination — done 2026-09-16, with `~/handoff-inbox/README.md` carrying the layout; what remains is that nothing yet tells a session to *read* the inbox, which is a user-scope `claude/CLAUDE.md` edit. Q3 (does delete-when-spent suit a client repo) and Q4 (does an index collide with a real tracker) stay open and **do not block drafting** — each directory's stub declares them. |
| [peer-session-coordination.md](peer-session-coordination.md) | **Open, written 2026-09-16.** The synchronous sibling of [handoff-convention-cross-repo.md](handoff-convention-cross-repo.md): that brief routes work between repos as *artifacts*, this one routes it between *live sessions*. The trigger is scale — fourteen sessions were live at 11:02 on 2026-09-16, **six of them in this working tree**, while `SendMessage` and `ListAgents` appear **nowhere in this repo** and every line of existing concurrent-session guidance treats another session as a hazard to avoid. Root `CLAUDE.md` already says "sequencing outranks branching when another session is live" and names no channel, because when it was written there was none. **Measured that day:** a session can spawn a cold `claude -p` in another repo in 11.5 s; it inherits user scope always and project scope **from its working directory**, so a cross-repo probe reads the *target* repo's `CLAUDE.md`, not the caller's; a real status query about `machine-config`'s gh-shim brief answered correctly at **$0.63** on the default model and **$0.12** on `haiku`. Two traps, both silent: **`--allowedTools` does not restrict** — a probe pinned to `Read Grep Glob` created a file anyway, since the flag is an allow-list for *auto-approval* and `defaultMode` is `auto`, whereas `--disallowedTools Write Edit NotebookEdit Bash` does block — and a spawned session in an **untrusted workspace silently drops that repo's `permissions.allow` entries**, whose named fix lives in `~/.claude.json`, the one file treated as runtime state rather than payload. The finding that sets the rule: a warm peer's payload view is **frozen at its start time**, since only `skills/` hot-reloads and everything else is a copy read once — so nine of the fourteen peers were running on user-scope instructions predating the 09:41 deploy, which is precisely why `machine-config-90` reported a landed change as still open. Its context predated it by eight hours; no diligence could have closed that. Hence **ask a warm peer only for what exists nowhere but in its head; for anything on disk, spawn cold** — and the inbox note and the peer message are not alternatives, the note being the durable artifact and the message the doorbell that stops it sitting unread. Proposes three edits and **no new skill** (`author-skill + learn` already scores 32.88 on the tokens such a description would use; `commit + land` only 11.82): a compact subsection in `claude/CLAUDE.md` beside the inbox paragraph, since discovery cannot be conditional; the missing channel in `commit`'s concurrent-session section; and a third row on `learn`'s Step 0 mode table. **v1 is read-only** — no peer-requested edits — which is *mechanically* enforceable on the cold-spawn channel and convention-only on the warm one, so the inbox stays the edit path. Four questions open, none blocking drafting. |
