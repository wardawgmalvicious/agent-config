...
| A Fabric tenant and a live `az login`, for the remaining MCP probes | [skills-for-fabric 2026-09-12 brief 01](../../audits/2026-09-12/skills-for-fabric/01-resite-the-hosted-fabric-mcp-placement-rule.md) D-3, for every hosted endpoint except `sqlEndpoint`. D-1 landed 2026-09-13 and **D-2 landed 2026-09-14**: `dataPlane/sqlEndpoint` connects from Claude Code via `headersHelper`, so that one is now in the project template and the four files asserting impossibility are corrected. Left are `core`, `powerbi`, unbound `dataPlane/kqlEndpoint`, the workspace/item-bound forms and the Activator reflex URL — unprobed, not refused, and the user chose on 2026-09-14 to come back to them. Method is that brief's log, not its D-2 text, which is stale in two ways: `claude mcp add` has no helper flag (use `add-json`) and `claude mcp list` health-checks from the shell, so no `/mcp` or restart is needed. **Carry the negative control** — a helper emitting a bogus bearer must yield the DCR error, or a "Connected" proves nothing — and confirm the login survives, since an interactive shell's profile runs `az account clear` and a cleared login produces that same DCR error on a working config. One open lead: bound URLs failed with `Error dialing …` rather than the DCR error under an absent credential, a different failure class worth isolating |
