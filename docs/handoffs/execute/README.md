...
Seven items remain on the queue: two open, four deferred, and one spent
but not yet deleted — see its row for what it is waiting on.
...
| [skill-overlap-script.md](skill-overlap-script.md) | **Spent 2026-09-15 — every part of its Kind has landed**, and it is kept only until the v1 output format has been used in anger. `scripts/skill-overlap.py` implements all four signals; `routing` gates commits as `lint-skill-routing`, with its negative case proved by `tests/scripts/skill-overlap/test-routing.sh` — the live tree passing says nothing, since a gate firing on nothing looks identical. The design fork went to the **shared module**, not the subcommand lean, on the lean's own test: three of the four signals take no repo argument, and `payload-coverage.py` is a flat parser whose first positional is `repos`. Scoped to one migrated caller rather than three. Consumers landed too — `author-skill` §2 and `learn` Step 4 — and neither gains retest debt, both already being `untested`. **What remains is the v1 proving and the body-duplication signal the brief put out of scope**; delete this row and the brief once the first has happened. |
