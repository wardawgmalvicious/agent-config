...
| [land-branch-cleanup.md](land-branch-cleanup.md) | **Open, written 2026-09-13.** `land` ends by reporting state, so every clean run leaves two refs behind; two runs in a consuming repo that day left four, removed by hand afterwards. The Constraints section already calls deleting the branch a default rather than a law without ever saying what the default is, so the fix is a **rewrite** of that clause plus a new step after verification — not an addition beside it. Proposes disclosing at the existing step 6 checkpoint rather than adding a third gate, and folding `delete_branch_on_merge` into the `gh api` probe step 7 already runs, since GitHub's auto-delete setting makes the remote half fail confusingly where it is on. Evidence is one personal repo with one contributor; whether the default holds in the client repos `workflow`-group deployment reaches is left to this repo. The accumulated-branch sweep is ruled out here as a different job. |
...
| Claude Code docs research, then a decision | [skills-for-fabric 11](../../audits/2026-09-10/skills-for-fabric/11-verify-claude-code-preregistered-oauth.md) — its **question 1 is answered**, superseded by the 2026-09-12 brief 01 below, which read both routes off first-party docs |
| A Fabric tenant, for one approved live MCP probe | [skills-for-fabric 2026-09-12 brief 01](../../audits/2026-09-12/skills-for-fabric/01-resite-the-hosted-fabric-mcp-placement-rule.md) D-2, with D-3 gated behind it. The prose half (D-1) landed 2026-09-13; what is left is the measurement it deliberately does not make. Put to the user that day and **left queued by choice**, not missed. Method is D-2 steps 1–4 verbatim — `az account show`, the `--query expiresOn` helper check alone, `fabric-sqlendpoint` at **local** scope, then `/mcp` — and a connect failure is as useful a result as a success. Traps: the helper is killed at 10 s and a cold `az` token can exceed that; a `401`/`403` gets exactly one retry; and a failed helper surfaces *as* the DCR error, which is the confusion D-1 exists to fix. D-3 then decides template placement per server by the existing workload-bound test, not by transport |
