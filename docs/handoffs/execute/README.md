...
| [environment-silent-wrong-answers.md](environment-silent-wrong-answers.md) | **Open, written 2026-09-22**, consolidating the environment halves of two inbox notes plus one declination candidate from a third. Four more members of a class root `CLAUDE.md` already collects without naming — **exit 0, plausible output, wrong** — beside the carriage-return count, `$TMPDIR`, `/tmp` and cp1252 stdout. **No IANA tzdata**: Git for Windows ships no `/usr/share/zoneinfo`, so a named `TZ` handed to `date` answers `GMT +0000` with no diagnostic, and a "7 days ago in the source system's zone" filter is off by the whole offset while looking right; Python's `zoneinfo` raises rather than lying, so only the bash half is in the silent class; the remedy is `uv run --no-project --with tzdata` (**required, not belt-and-braces**, ~2.6 s cold) or `[TimeZoneInfo]` from `pwsh`, which resolves IANA ids with nothing installed. **`Path.write_text()` on Windows writes CRLF and `read_text()` hides it** — a six-line edit staged as 440 insertions / 408 deletions while an in-Python CR assertion passed, because text-mode reads translate the CRs back before the check sees them; there is no general "git will catch it", since `.gitattributes` can leave a path `unspecified`. **GitHub API timestamps are UTC**, so a date derived from `mergedAt` is wrong for the last four or five hours of every local day — caught once only because it contradicted merge times quoted in the same document. Plus **a secret never travels on argv**, explicitly *not* of that class and flagged as a design decision carried on the user's choice rather than a measured failure, wanting a `## Secrets` heading `coding-bash.md` does not have. Item 5 — pipeline exit codes, where `grep` returning 1 on no match inverts every check of an absence — is listed **so it can be declined on the record** rather than rediscovered as a gap. The sentence worth more than any single member: where a translation layer sits on both the read and the write, a round-trip check proves nothing — go under it, or check from the other side. |
