...
Three briefs are open. Fifteen waves are spent (struck through below).
...
deleted **individually** as each is spent, and heavily cross-linked — 32
relative markdown links across the current 4 files, counting this queue
(recounted 2026-09-03 when wave 16 retired
`logicalid-runtime-id-encoding.md`, by the method kept with the number:
...
well; earlier figures of 27 and 41 do not reproduce under any method
...
late only because it was expensive. Of what remains, **7 and 17 block nothing and
are blocked by nothing** — take whichever fits the session you have. 16
was the third, and it is spent: it was gated only on a credential, and
the session that had one closed it.
