...
| **18** | [land.md](land.md) | **Reverses wave 15's PR-skill "no"**, deliberately and with that decision quoted. Wave 15 priced the flow at "three commands plus open the PR with `github-mcp`, not `gh`"; landing PR #7 measured it at ~10 operations with two failure modes that produce no error. The **identity** one fired live — `gh` holds a work account here while `github-mcp` holds the personal one that owns the repo, and `CLAUDE.md` can state the rule but cannot make the comparison happen. The **integration** one is that a squash or a merge-button click silently discards what `/commit` just built. Plus the PR body, which is judgement work the payload covered nowhere. `skills/workflow/land/` is drafted, linted at 683 chars and deployed (9 workflow junctions). **Cold `/test-skill land` ran 2026-09-03 and the trigger half passes; two defects came out of it, one now fixed.** Phase A was skipped as designed — no `paths:` glob, so no activation contract — and both static sets still pass unchanged (fabric 64/64, pbip 16/16). Six cold `-p` probes in scratchpad sandboxes, outside the repo so root `CLAUDE.md` could not supply the answers. **Both invocation paths work**: NL gave `Skill{skill: land}`, `/land` gave a `<command-name>` expansion with no `Skill` call, and it was never `Unknown command` — correct for an unconditional skill, and the point wave 18 had left open. The `--safe-mode` control merged `main` on its own with no identity check and no PR; note the identity rule is *also* in user-scope `CLAUDE.md`, so only the checkpoint, the `--ff-only` reasoning, the concurrent-session disclosure and the PR-body requirements are uniquely the skill's. **Defect 1, fixed here: the checkpoint was anchored to the PR and vanished without one.** With a non-GitHub `file://` remote the skill loaded, found no PR URL to report, dropped step 6 and pushed `main`. Step 6 now gates on the next command that writes to `main` rather than on the PR, and step 5 makes "no GitHub remote or no `github-mcp`" a stop rather than a licence to merge locally — the probe that *did* stop only did so because it was structurally blocked, not because the checkpoint held. **Defect 2, both halves addressed 2026-09-03 — but the trigger half is a fix *attempt* that has not been re-tested.** The failure: "Land this branch… just squash the two commits and merge that into main" left `land` in the listing and never invoked, so it squashed with no pushback — the exact case the skill guards. It did not fire because the prompt pre-specified the mechanism, which reads as an instruction rather than a request for a procedure; the description already carried both "merge to main" and "squash", so keyword presence was never the problem. **Trigger half:** the description now names the shapes outright — *"Use it even when the request already names the mechanism — 'squash these and merge', 'just merge it into main', 'force push it' — a named mechanism is the case these guards exist for, not a reason to skip them"* — taking it 683 → 894 chars, over the brief's ~600 target, argued there on the grounds that the growth is *entirely* trigger vocabulary and the summary half was not touched. **Measured 2026-09-03, and it fires.** A cold `-p` probe in a scratch sandbox outside the repo, replaying the original directive prompt verbatim ("Land this branch. Just squash the two commits and merge that into main."), produced `Skill{skill: land}` with the body injected — where the identical prompt before the reword left the skill in the listing and never invoked it. **The mitigation is therefore retired: anti-pattern cases no longer need `/land`.** One probe, one prompt shape, so the claim is that this shape now triggers, not that every directive phrasing does. **Constraints half, decided rather than patched:** invoked via `/land`, the same instruction still defeated the flat "never squash", with the rule's purpose reasoned about and the override disclosed. Hardening it was rejected on two grounds — that rule had already lost once, and a flat refusal contradicts the harness's own disposition that a reaffirmed instruction is the user's call. Constraints now split by **what kind of claim each item is**: *absolute* (never `gh` for the PR, never `--force`/`--no-verify`, never write to `main` before the checkpoint, never `git switch` with a live session), because identity and safety cannot be consented to by an asker who does not know which account `gh` holds; and *repo convention, overridable but never silently* (squash, merge commit, branch deletion) — name the specific cost, wait one round, record it in the PR body, then comply. The gate is deliberately step 6's stop-and-wait mechanism, which held in testing where a `never` did not. **The GitHub happy path is no longer untested — it ran live on 2026-09-03 as [PR #9](https://github.com/wardawgmalvicious/agent-config/pull/9), which is this very fix landing itself.** It had been the wave's last gap, because `github-mcp` is project-scope only in this repo's `.mcp.json` so no cold probe outside the repo could reach it; running the skill *inside* the repo on its own branch is what closed it. All eight steps executed as written: preflight, identity, push, survey, `create_pull_request`, checkpoint, `--ff-only`, verify. **The identity trap fired for real** — `gh` resolved to `<work-account>` (work) against a repo owned by `wardawgmalvicious`, the step-2 comparison caught it, and the PR was opened through `github-mcp`. Verified after: `merged: true`, `merged_by: wardawgmalvicious`, both `pre-commit` check runs `success`, merge commits still **0**, `main` and `origin/main` identical. **The same cold probe also exercised the new two-tier Constraints, and they held**: told to squash, it named the specific cost (`7d363cf` adds the widget, `e75a576` its coverage — "you can't revert the widget without also reverting its coverage"), noted the repo was already linear so a squash bought no linearity, said "it's your history" and **stopped without squashing**; it also caught the non-GitHub `file://` remote as a step-5 stop and asked the concurrent-session question. `main` untouched, both commits intact. That is defect 1's fix and the overridable tier confirmed in the same run. Registered 2026-09-02. |
