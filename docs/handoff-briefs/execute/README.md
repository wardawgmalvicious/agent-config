...
Eight briefs are open. This file is the **only** place the order lives;
...
deleted **individually** as each is spent, and heavily cross-linked — 21
links between the current 8 files. Numbering the filenames would mean
...
dependency and by cost of delay.

| # | Brief | Why here |
| --- | --- | --- |
| **0** | [fixture-shape-capture.md](fixture-shape-capture.md) | **Start first, finish last.** Needs the user to spin up `.DataAgent` / `.SQLDatabase` / `.GraphModel` items — external lead time, tiny effort. Kick it off, then carry on; it unblocks wave 4. |
| **1** | [rule-glob-gaps.md](rule-glob-gaps.md) — bugs 1, 1b | **The only live bug in the queue.** `coding-sparksql.md` has never fired on a Fabric notebook and `coding-tsql.md` matches instead, so Spark SQL is getting T-SQL conventions today. Everything else here is an improvement; this is wrong output. |
| **2** | [item-type-skill-kqlqueryset.md](item-type-skill-kqlqueryset.md) + [rule-glob-gaps.md](rule-glob-gaps.md) bug 2 | One decision in two files — whether KQL conventions should reach the JSON files that hold queries. Doing them apart means deciding twice. |
...
| **4** | [skill-context-cost.md](skill-context-cost.md) workstreams C + E, folding in [item-type-skill-lakehouse.md](item-type-skill-lakehouse.md) | The big pass. Lakehouse **is** an E row — its recommended fix is a `paths:` glob on `fabric-variable-library` — so run them together or E gets made twice. Wants wave 0 done first: E's `paths:` candidates should be measured against confirmed item-type names. |
...
| **6** | [rule-glob-gaps.md](rule-glob-gaps.md) — bug 3 | Blocked on wave 0. Adding `**/*.GraphModel/**` to `fabric-git-serialization.md` before the name is confirmed propagates the error into a second file. |
...
**Bugs before improvements.** Wave 1 is the only thing in the queue
producing wrong behaviour right now.

**Lead time before effort.** Wave 0 is minutes of work behind days of
waiting, so it goes first despite being small and blocking only wave 6.
...
[item-type-skill-kqlqueryset.md](item-type-skill-kqlqueryset.md) and
[item-type-skill-datapipeline.md](item-type-skill-datapipeline.md) each
open with a recommendation, and two of them recommend **not** building the
