...
3. **Get the changes** — two strategies, by commit count:
   ...
   - **More than 5** — `get_file_contents` at the prior ref (resolve it with **Resolving the diff base** in § 4b — that rule governs this path too, and resolving it by commit order instead of by `path` silently drops changes) and again at `<head-sha>`, then diff the two versions as in step 4c. Cheaper than a patch per commit once the count climbs. Where the source's `path` is a **directory**, `get_file_contents` on it returns a listing rather than content — fetch each name in the entry's `files` list instead, at both refs, after narrowing that list per step 4.
...
GitHub anonymous API limit is 60 requests/hour and this path is unauthenticated. **Price the selected set before running it, and price it in calls rather than sources.** A single-file source costs ~3 (commits list, raw at prior ref, raw at HEAD) plus one re-fetch per `sections` entry when the completeness check below trips; a **directory** `path` costs `1 + 2 x |files|` instead. Source count is therefore a bad proxy — one directory source with a long `files` list outweighs three single-file ones, and this line previously read "the four registered sources fit; a registry past roughly six does not," which was wrong on both the count and the driver.

Worked once, at 2026-09-08, against the registered set: `fabric` up to 14 (3 + its 11 named sections and workload subsections), `vscode-agent` 9 (4 files), `fabric-iq-ontology` 13 (6 files), `powerbi` ~0 GitHub calls — its entry override degrades to the Learn page, not to raw URLs — and `claude-code` 0, not attempted. That is ~36 of 60 worst case, so the set fits with less headroom than a per-source budget suggests. Re-derive it rather than trusting that figure. Once a run's projected calls approach the limit, the `github-mcp` path stops being optional.

A source the registry marks github-mcp-only — `claude-code` today — is not attempted on this path at all: name it in the report's skipped list rather than diffing a summary of it. A source whose entry defines its own fetch strategy is priced by that strategy and not by this rule.

**Resolving the diff base — one rule, and it governs both fetch paths.** If the user supplied a SHA whose repo matches this source, use that SHA directly. If a SHA from another source's repo, fall back to the date resolution. For a date, the base is **the last commit that touched the source's `path` before the floor** — `list_commits` with that `path`, `until: <floor-date>T00:00:00Z`, `perPage: 1`, `fields: ["sha"]`.

**Not "the parent of the oldest in-window commit"**, which is what this step said until 2026-09-07 and is wrong on any repo that merges release branches. The oldest in-window commit is typically the monthly release *merge*, so its first parent is a same-day main-branch commit that never touched the audited file and is dated *inside* the window. Measured on `powerbi` that day: the old rule resolved `f1f53694` (2026-08-20, touching `service-get-started.md` and `power-bi-visualization-card.md` — not `whats-new.md`), where the correct rule resolves `443eb78f` (2026-07-22, the July release merge, which is genuinely the file's previous version).

It survived undetected because it produced an identical diff — nothing else touched that file in between, so the two bases are byte-identical for `path`. That is luck, not correctness. Had any in-window commit touched the audited file *before* the release merge, its change would sit in the **base** and vanish from the diff: a silent under-report with no error path and no symptom in the output. Resolve the base by path, always.
...
- **MCP match** — `Read ~/.claude/mcp/.mcp.global.template.json` **and** `~/.claude/mcp/.mcp.project.template.json` for the current server inventory across both global and per-project scopes; identify whether the entry adds, removes, or changes a server. Placing a finding is a two-step test. First, does it work from Claude Code at all? The Fabric-hosted `api.fabric.microsoft.com/v1/mcp/*` endpoints do not connect through Claude Code's **automatic** OAuth flow (DCR), but a `headersHelper` command supplying an `az` bearer does reach them — **every endpoint probed as of 2026-09-14 (CLI 2.1.268) connects**: bare `dataPlane/sqlEndpoint` and `dataPlane/kqlEndpoint`, `core`, `powerbi`, a workspace/item-bound `kqlEndpoint`, and the workspace-bound Activator reflex URL. So the hosted endpoints are **not** categorically workspace-template-only — place each one by measurement, and treat "hosted" as saying nothing about reachability. The single measured failure is the workspace/item-bound `dataPlane/sqlEndpoint`, which returns `MCP endpoint not found` in the one tenant probed while its bound `kqlEndpoint` sibling connects; the second documented route, `--client-id`/`--client-secret`, is still unprobed. An unprobed endpoint still defaults to `.vscode/mcp.template.json` in the agent-config repo. Note that a helper producing no credential prints the DCR error itself, so that error is not evidence the route is unsupported — see `claude/mcp/README.md`. Second, if it does work: is it bound to a workload (needs a workspace ID, database, connection string, or a running desktop app) or not? Workload-bound goes in the project template; cross-workload — docs, source control, cloud control plane — goes in the global one. Transport is not the test; a stdio server can be workload-bound and an http server can be universal.
